...
// demedallod is the official command-line client for deMedallo.
package main
...
import (
	...
	"github.com/demedallo/demedallod/metrics"
)
...
var versionCommand = cli.Command{
	Action: version,
	Name:   "version",
	Usage:  "Print deMedallo version numbers",
	Description: `
...
var makeMlogDocCommand = cli.Command{
	...
	so 'demedallod mdoc -md discover' will generate markdown documentation only
...
func makeCLIApp() (app *cli.App) {
	...
	app.Version = Version
	app.Usage = "the demedallod command line interface"
	app.Action = geth
	...
	app.Before = func(ctx *cli.Context) error {
		...
		if ctx.IsSet(SputnikVMFlag.Name) {
			if core.SputnikVMExists {
				...
				log.Fatal("This version of demedallod wasn't built to include SputnikVM. To build with SputnikVM, use -tags=sputnikvm following the go build command.")
			}
		}
	...
	app.CommandNotFound = func(c *cli.Context, command string) {
		fmt.Fprintf(c.App.Writer, "Invalid command: %q. Please find `demedallod` usage below. \n", command)
		cli.ShowAppHelp(c)
		os.Exit(3)
	}
	return app
