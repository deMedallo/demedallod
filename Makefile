...
# Choose to install demedallod with or without SputnikVM.
WITH_SVM?=1
...
build: cmd/abigen cmd/bootnode cmd/disasm cmd/ethtest cmd/evm cmd/gethrpctest cmd/rlpdump cmd/demedallod ## Build a local snapshot binary versions of all commands
	@ls -ld $(BINARY)/*

cmd/demedallod: ## Build a local snapshot binary version of demedallod. Use WITH_SVM=0 to disable building with SputnikVM (default: WITH_SVM=1)
	if [ ${WITH_SVM} == 1 ]; then ./scripts/build_sputnikvm.sh build ; else mkdir -p ./${BINARY} && go build ${LDFLAGS} -o ${BINARY}/demedallod ./cmd/demedallod ; fi
	@echo "Done building demedallod."
	@echo "Run \"$(BINARY)/demedallod\" to launch demedallod."
...
install: ## Install all packages to $GOPATH/bin
	...
	$(MAKE) install_demedallod

install_demedallod: ## Install demedallod to $GOPATH/bin. Use WITH_SVM=0 to disable building with SputnikVM (default: WITH_SVM=1)
	$(info Installing $$GOPATH/bin/demedallod)
	if [ ${WITH_SVM} == 1 ]; then ./scripts/build_sputnikvm.sh install ; else go install ${LDFLAGS} ./cmd/demedallod ; fi
...
.PHONY: setup test cover fmt lint ci build cmd/demedallod cmd/abigen cmd/bootnode cmd/disasm cmd/ethtest cmd/evm cmd/gethrlptest cmd/rlpdump install install_demedallod clean help static
