...
var (
	...
	Use "$ demedallod dump 0" to dump the genesis block.
		`,
	}
...
// $ demedallod dump [hash|num] [hash|num] ... [hash|num]
// $ demedallod dump 0x234234234234233 42 43 0xlksdf234r23r234223
//
...
// revised use: $ demedallod dump [hash|num],[hash|num],...,[hash|num] [address],[address],...,[address]
//
// Added unsorted/sorted dumping algorithms.
// unsorted dump is used by default.
// revised use: $ demedallod dump [sorted] [hash|num],[hash|num],...,[hash|num] [address],[address],...,[address]

func dump(ctx *cli.Context) error {

	if ctx.NArg() == 0 {
		return fmt.Errorf("%v: use: $ demedallod dump [blockHash|blockNum],[blockHash|blockNum] [[addressHex|addressPrefixedHex],[addressHex|addressPrefixedHex]]", ErrInvalidFlag)
	}
