...
if [ ! "$OUTPUT" == "build" ] && [ ! "$OUTPUT" == "install" ]; then
	...
	echo "With SputnikVM, running demedallod $OUTPUT ..."
fi
...
if [ "$OS" == "Windows" ]; then
    ...
    cd %GOPATH%\src\github.com\demedallo\demedallod\cmd\demedallod
	...
	if [ "$OUTPUT" == "install" ]; then
		...
		go build -ldflags '-X main.Version='$(git describe --tags) -o %GOPATH%\src\github.com\demedallo\demedallod\cmd\demedallod -tags=sputnikvm .
	fi
else
	...
	demedallod_binpath="$ep_gopath/demedallod/bin"
	echo "Doing demedallod $OUTPUT ..."
	cd "$ep_gopath/demedallod"
    if [ "$OS" == "Linux" ]; then
		if [ "$OUTPUT" == "install" ]; then
			CGO_LDFLAGS="$sputnikffi_path/c/libsputnikvm.a -ldl" go install -ldflags '-X main.Version='$(git describe --tags) ./cmd/demedallod
		elif [ "$OUTPUT" == "build" ]; then
			mkdir -p "$demedallod_binpath"
			CGO_LDFLAGS="$sputnikffi_path/c/libsputnikvm.a -ldl" go build -ldflags '-X main.Version='$(git describe --tags) -o $demedallod_binpath/demedallod -tags=sputnikvm ./cmd/demedallod
		fi
    else
		if [ "$OUTPUT" == "install" ]; then
			CGO_LDFLAGS="$sputnikffi_path/c/libsputnikvm.a -ldl -lresolv" go install -ldflags '-X main.Version='$(git describe --tags) -tags=sputnikvm ./cmd/demedallod
		elif [ "$OUTPUT" == "build" ]; then
			mkdir -p "$demedallod_binpath"
			CGO_LDFLAGS="$sputnikffi_path/c/libsputnikvm.a -ldl -lresolv" go build -ldflags '-X main.Version='$(git describe --tags) -o $demedallod_binpath/demedallod -tags=sputnikvm ./cmd/demedallod
		fi
    fi
fi
