...
import (
	...
	"github.com/demedallo/cryptonight"
	"github.com/demedallo/demedallod/accounts"
	"github.com/demedallo/demedallod/common"
	"github.com/demedallo/demedallod/core"
	"github.com/demedallo/demedallod/core/state"
	"github.com/demedallo/demedallod/core/types"
	"github.com/demedallo/demedallod/crypto"
	"github.com/demedallo/demedallod/eth"
	"github.com/demedallo/demedallod/ethdb"
	"github.com/demedallo/demedallod/event"
	"github.com/demedallo/demedallod/logger"
	"github.com/demedallo/demedallod/logger/glog"
	"github.com/demedallo/demedallod/miner"
	"github.com/demedallo/demedallod/node"
	"github.com/demedallo/demedallod/p2p/discover"
	"github.com/demedallo/demedallod/p2p/nat"
	"github.com/demedallo/demedallod/pow"
	"github.com/demedallo/demedallod/whisper"
...
// for demedallod and returns half of the allowance to assign to the database.
...
func makeNodeName(version string, ctx *cli.Context) string {
	name := fmt.Sprintf("demedallod/%s/%s/%s", version, runtime.GOOS, runtime.Version())
...
func MakeSystemNode(version string, ctx *cli.Context) *node.Node {
	...
	if ctx.GlobalBool(Unused1.Name) {
		glog.V(logger.Warn).Warnln(fmt.Sprintf("demedallod started with --%s flag, which is unused and can be omitted", Unused1.Name))
	}
...
func mustMakeSufficientChainConfig(ctx *cli.Context) *core.SufficientChainConfig {
	...
	if _, de := os.Stat(defaultChainConfigPath); de != nil && os.IsNotExist(de) {
		...
		$ demedallod --chain morden dump-chain-config %v/chain.json
...
func logChainConfiguration(ctx *cli.Context, config *core.SufficientChainConfig) {
	...
	glog.V(logger.Info).Infof("Starting demedallod %s", ctx.App.Version)
	glog.D(logger.Warn).Infof("demedallod version: %s", logger.ColorGreen(ctx.App.Version))

	glog.V(logger.Info).Infof("demedallod is configured to use blockchain: %v", config.Name)
