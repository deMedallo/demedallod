...
func status(ctx *cli.Context) error {
	...
	if !shouldUseExisting {
		glog.D(logger.Warn).Warnln("demedallod has not been initialized; no database information available yet.")
		return nil
	}
...
func gpubench(ctx *cli.Context) error {
	args := ctx.Args()
	wrongArgs := func() {
		glog.Fatal(`Usage: demedallod gpubench <gpu number>`)
	}
	switch {
	case len(args) == 1:
