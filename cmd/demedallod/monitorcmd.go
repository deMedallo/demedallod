...
var (
	...
	monitorCommand = cli.Command{
		Action: monitor,
		Name:   "monitor",
		Usage:  `deMedallod Monitor: node metrics monitoring and visualization`,
		Description: `
	The demedallod monitor is a tool to collect and visualize various internal metrics
...
func monitor(ctx *cli.Context) error {
	...
	if len(monitored) == 0 {
		...
		if len(list) > 0 {
			...
			log.Fatal("No metrics collected by demedallod (--metrics).")
		}
	}
...
// retrieveMetrics contacts the attached demedallod node and retrieves the entire set
...
// eg.
// $ demedallod monitor [--attach=api-endpoint] metric1 metric2 ... metricN
//
